...
class JazShopEasyformOrders(BrowserView, DateMixin):
    ...
    def __call__(self):
        ...
        easy_form = self.context.aq_parent    # XXX TEST THIS !!!!
        form_uid = IUUID(easy_form)

        # Filter orders to those generated by this form
        orders = [o for o in order_list
                           if form_uid in o.get('easy_forms', {})] # XXX WE MIGHT JUST NEED TO KEEP THIS AS pfg_forms for compatibility. or maybe not ??
        orders_csv = StringIO()
        ...
        if orders and len(orders) > 0:
            ...
            for order in orders:
                ...
                ldict.update(order['easy_forms'][form_uid])  # XXX WE MIGHT JUST NEED TO KEEP THIS AS pfg_forms for compatibility. or maybe not ??
                writer.writerow(ldict)
